...
- **`options.children` is read once at `GridStack.init()` and never re-synced** — confirmed by reading `updateOptions()`'s own source: it reacts to scalar grid-level options (`cellHeight`, `margin`, `staticGrid`, etc.) but never touches `children` on a later call. This is why `DashboardPage.vue`'s `gridOptions` computed can safely recompute `children` from `dashboard.value.widgets` on every 30s poll tick without causing disruptive re-renders (gridstack just ignores it) — but also why runtime add/remove **must** go through the raw grid instance's own imperative `addWidget()`/`removeWidget()` (via `getGrid()` on a template ref for add in `DashboardPage.vue`, via the `useGridStack()`/`useGridStackItem()` composables for remove in `DashWidget.vue` itself, both confirmed real by reading `gridstack/dist/vue`'s source directly rather than assuming). `staticGrid: !editing`, by contrast, genuinely *is* re-read reactively by `updateOptions()` — confirmed the same way — so the Edit layout/Done editing toggle needed no special-case code at all, just falls out of the existing computed.
