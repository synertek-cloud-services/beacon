...
- **Real-hardware verification, one real bug found and fixed**: dispatched against a real Windows box, the service and its registry entry both cleanly disappeared, but `C:\Program Files\Beacon` was left behind and the tray icon was still visibly running afterward. Root cause: `beacon-tray.exe` runs as a *separate* process launched into the logged-in user's own session (`usersession.RunAsSession`), not a child of the service being stopped — stopping the service does nothing to it, and its own open file handle on `beacon-tray.exe` blocks `rd /s /q` from removing the directory. Fixed by adding `taskkill /IM beacon-tray.exe /F` to the helper script before the `rd /s /q` step (SYSTEM's `SeDebugPrivilege` lets it kill a process in a different session; `/IM` matches every instance across every session in one call, covering the RDS/multi-session case too; chained with `&` not `&&` so cleanup still proceeds when no tray process exists to kill). The self-referential stop/restart race the design was actually built to avoid was never hit live — the bug that did surface was a different, simpler oversight (a second unrelated process holding a file lock), the kind only real hardware turns up.
