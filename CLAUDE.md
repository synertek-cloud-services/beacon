...
**Consent parity with Web Remote, confirmed as the design**: Datto RMM's own "Privacy Mode" (a same-name-but-different-thing from Beacon's own Privacy Mode, which only limits audit inventory collection -- see Privacy Mode's own section) is actually the same concept Beacon already built as Web Remote's end-user consent (issue #86, migration `0083`): an Accept/Decline prompt gating a remote connection, applying uniformly across every remote tool in Datto's Agent Browser, not just one. RustDesk connections don't route through Beacon's relay at all (a technician dials the RustDesk ID directly with their own separate RustDesk client), so Beacon can't inject a prompt into that handshake the way `beacon-screenshare.exe` does for `screen_share` sessions -- but RustDesk has its own equivalent primitive: a **"Click" approve mode** (an actual human has to approve the connection) as opposed to permanent-password mode, plus a documented override that falls back to the permanent password specifically when nobody's logged in (locked/logon screen) since there's no one present to click anything -- the same "unprompted when there's nothing to protect" logic already established for Web Remote's SYSTEM fallback. The install pipeline should configure RustDesk into Click mode (with the logon-screen-password override) when the device's existing `remote_access_consent_override ?? company.remoteAccessConsentRequired` resolves true, and plain permanent-password mode when it resolves false -- reusing that exact toggle rather than adding a second, RustDesk-specific consent setting. The precise config key names/`--config` fields for approve-mode aren't pinned down yet (concept confirmed via RustDesk's own docs/GitHub discussions, exact flags not yet verified) -- verify against the real client before building.
