...
- `agent/cmd/beacon-tray` (Windows-only) — a tray icon helper, and the first real consumer of `usersession`. Deliberately staged the same way `usersession` itself was: **Step 1 only** — the tray binary itself (`fyne.io/systray`, this codebase's first GUI-toolkit dependency; icon at `assets/icon.ico`, generated from the existing `dashboard/public/brand-mark.svg` — note the source SVG's non-zero `viewBox` origin (`138 23 240 418`) tripped up `oksvg`'s `SetTarget`/ViewBox-offset handling during generation, worked around by normalizing the SVG to a `0 0 …` viewBox first rather than trusting the library's offset math), shown with a static, non-interactive menu (version label + a "Visit Dashboard" link) and deliberately **no Exit item** and **no real launch lifecycle wired up yet** — no session-change hook, no supervision, not embedded into `beacon-agent.exe`, not part of `scripts/publish-agent.mjs`. Datto RMM's own tray process (`gui.exe`, confirmed via their docs to run "in every user session") doesn't have its actual launch mechanism publicly documented, so the eventual Step 2 design (session-change hook + periodic health-check relaunch, `beacon-tray.exe`'s bytes embedded in `beacon-agent.exe` via `//go:embed` rather than adding a second release artifact) is an engineering recommendation, not a confirmed copy of Datto's internals.
