...
Builds all 5 platform/arch binaries, signs each with `BEACON_SIGNING_KEY`, and registers each with the worker via `POST /v1/admin/agent/versions` — needs `BEACON_WORKER_URL`, `BEACON_ADMIN_SECRET`, `BEACON_SIGNING_KEY` env vars. **Must be run from the repo root**, not `agent/` — it resolves `scripts/publish-agent.mjs` and `agent/` relative to its own script location, but the working directory still matters for the relative build paths it shells out to.

`BEACON_DOWNLOAD_BASE_URL` (e.g. `https://github.com/<org>/<repo>/releases/download/v<version>`) must be set to the real GitHub Release's asset base — omitting it falls back to a dead placeholder URL that registers successfully but 404s on every real agent download. Set it once the release exists (`gh release create vX.Y.Z dist/beacon-agent-*` first, same as always).

**Standard release flow** (build/upload needs no secret; signing does, so it's a separate step on whatever machine actually holds `BEACON_SIGNING_KEY`): build all 5 binaries and create the GitHub Release first — this part is safe to run from anywhere, including in-session. Sign+register happens afterward, on the machine with access to the password manager, by downloading the *exact already-uploaded* assets (`gh release download vX.Y.Z`) and signing those files directly — **never re-running `publish-agent.mjs`'s build step for this**, since without `-trimpath` a Go binary embeds its absolute build path, so a rebuild from a different machine/directory produces different bytes than what's already hosted, and the freshly-computed signature won't match the uploaded asset.

**Verify a release actually works end-to-end** via the **unauthenticated** `GET /v1/agent/version?os=&arch=&current=` and `GET /v1/agent/download?os=&arch=` endpoints (`worker/src/routes/agent-update.ts`) — a `curl -sIL` through `/v1/agent/download` should end in `HTTP/2 200`, not a 404. This is necessary but **not sufficient** — it proves the download resolves, not that the signature is valid.

**`BEACON_SIGNING_KEY` has no `.env`/secrets-manager home in the current workflow — it's manually re-entered each release from the password manager**, which is a real, repeated source of paste errors (nothing in `tools/sign` validates that the entered key's derived public half matches `pinnedPublicKey`, so a wrong key signs successfully and every step — build/sign/register/download — reports success anyway; the only symptom is real devices silently never updating, invisible on Windows since services have no console). **Standing practice: independently verify every release's signature before considering it shippable** — download the real GitHub release asset, re-run the SHA-256 → `ed25519.Verify` check against `pinnedPublicKey` (`agent/internal/updater/verify.go`) in a throwaway program, pulling the registered `signature_hex` programmatically rather than hand-transcribing it. **Open question, not yet resolved**: whether the manual-paste-per-release step itself should be replaced with something that doesn't require re-entering the key each time (a password-manager CLI integration, a locally-encrypted key file, etc.) — worth revisiting given how often this step alone has derailed a release.
