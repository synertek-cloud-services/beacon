...
Windows Update integration, built in deliberate slices matching this codebase's established scoping pattern (see Components/Job System's ComStore, Device Groups, etc.): **Patch Visibility** (scan+report only), then **approval workflow** (fleet-wide approve/ignore), then **install capability** (actually installing approved patches). A **Patch Policy** type (auto-approval rules + scheduled recurring install windows, modeled on Maintenance Policy) is scoped but not built — see below.
...
- **Reboot handling researched directly against Datto RMM** (the actual reference product, not assumed): Datto's real Patch Policy reboot setting is Restart / Shut down / Do not restart (default), where "Do not restart" additionally supports an end-user notification with up to 99 postponable reminders. At the time this was built, Beacon's agent had no user-impersonation path (same gap as Components/Job System's declined "run as a logged in user"), so the notification/postpone mechanic was a hard no. That underlying primitive exists now (`agent/internal/usersession`, see Architecture below) but a reboot-prompt UI built on top of it hasn't been — a real, separate follow-up. Install capability itself doesn't reboot anything; it only reports `reboot_required` per run. The existing `reboot` command type already covers triggering one manually. An opt-in "reboot if needed" toggle is scoped as part of Patch Policy below, not built standalone.

### Patch Policy — scoped, not built
Auto-approval rules (a `min_severity` threshold — deliberately simple, no category/KB-pattern filter builder, matching how CPU/memory monitors use a plain threshold rather than a rule engine) plus scheduled recurring install windows, modeled on Maintenance Policy's targeting/recurrence shape (`maintenance_policies`, `worker/src/lib/maintenance.ts`, migration `0050`) but **not** its runtime model: Maintenance Policy is a passive suppression gate checked on-demand at alert-evaluation time; Patch Policy would need to *actively dispatch* `install_patches` commands at each scheduled window, closer to Jobs' `dispatchDueScheduledJobs` cron-dispatch shape (`worker/src/routes/admin/jobs.ts`) than to Maintenance Policy's `isMaintenanceWindowActive`. Would need its own "already dispatched for this window" bookkeeping — `dispatchDueScheduledJobs`'s `NOT EXISTS (SELECT 1 FROM commands WHERE job_id = j.id)` trick doesn't directly translate since there's no `jobs` row here. An `auto_reboot` bool would reuse the existing `reboot` command type, defaulting off (matching Datto's own "Do not restart" default). Also out of scope regardless: patch rollback/uninstall on failure, per-update retry logic, any Shared Dashboards widget.
