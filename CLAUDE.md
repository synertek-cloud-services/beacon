...
- **Not yet tuned, real follow-ups once live feedback comes in**: `float: false` (auto-compaction on) vs `float: true` (allow gaps) was picked as the default without being able to eyeball it live — a real product-feel call, not a settled one. Mobile/narrow-viewport behavior (the old system had a `@media (max-width:760px)` stacked-block fallback; gridstack supports responsive `columnOpts` for the same effect but it isn't wired up yet). The default `'se'`-only resize handle's default grey SVG cursor icon wasn't restyled for contrast against the dark theme. **None of this has been verified in a real browser** — drag, resize, persistence-after-reload, and the "×" remove/add-widget paths all need real hands-on testing before this is trustworthy, the same limitation flagged for every other piece of interactive UI work this session couldn't test directly.
