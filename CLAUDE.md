...
- **Frontend not yet built** — `dashboard/src/brand.ts` (new, mirrors `auth.ts`'s reactive-singleton shape, not `theme.ts`'s stateless-DOM-mutation shape, since this needs real template bindings), wiring into `App.vue`'s sidebar mark/name and `LoginPage.vue`'s brand lockup, and a new section on `BrandingSettingsPage.vue`, are all designed but deliberately deferred — the user had unrelated dashboard work in flight when this was built. Only a single **Product Name** field exists (no separate Company Name) — confirmed with the user that Beacon has exactly one real UI surface to drive today (sidebar/login/tab title), and a second field with no effect would be the same "fake toggle" problem already avoided elsewhere in this codebase.
