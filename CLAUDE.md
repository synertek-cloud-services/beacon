...
- `agent/cmd/beacon-tray` (Windows-only, embedded into the agent binary via `//go:embed`, extracted/launched per active session by `agent/internal/service`'s `EnsureTrayRunning()`) — a tray icon built on a **local fork of `fyne.io/systray`** (`agent/internal/vendor/systray`, Apache-2.0, wired in via a `replace` directive in `agent/go.mod` — see that package's own doc comment for the two fork additions and how to re-sync from a newer upstream release), launched into every active session (RDS/AVD/multi-session aware) via `usersession`. Shows version, a "Visit Dashboard" link (when `--dashboard-url` is set — currently never, since the agent doesn't know the dashboard's origin) and a "Get Support" link (when `support_url` is configured — see Branding's Identity subsection). `service.Install()`/`Uninstall()`/`SelfUninstall()` all `taskkill /IM beacon-tray.exe /F` before removing the install directory (a running tray process's file lock otherwise blocks removal).

  **Tray↔service IPC** (`agent/internal/traypipe`) — a `github.com/microsoft/go-winio` named pipe (`\\.\pipe\BeaconAgent`, SDDL-locked to local `BUILTIN\Users` read+write, network access explicitly denied, SYSTEM/Administrators full control), replacing three earlier ad-hoc mechanisms: launch-time-only CLI flags (version/support-URL were only ever "eventually consistent," picked up on a tray's next relaunch), a polled `pending-reboot.json` file requiring a hand-granted `icacls` ACL exception for the unprivileged tray to write back to (root cause of a real production incident — a device's grant silently never took effect and stayed stuck for 16 days, invisible until a technician inspected the raw ACL by hand), and bare-PID-number liveness tracking. The SYSTEM agent service listens; each tray instance dials in and sends a `hello` identifying its session, after which `version_info` (live-pushed on every `SetAgentVersion`/`SetSupportURL` call, not just at relaunch) and the reboot confirm/snooze handshake (`reboot_prompt`/`reboot_response`, replacing the old `MessageBox`-writes-a-file flow — see `agent/internal/service/reboot_windows.go`) both flow over this one connection. `EnsureTrayRunning()`'s reconciliation now treats a live pipe connection as the primary "is this tray actually alive" signal; the old PID/`processAlive` check is kept only as a bootstrap fallback for the brief pre-connection window. The one piece of reboot state that must survive an agent *service* restart is a small SYSTEM-only breadcrumb (`credential.Dir()/reboot-pending.json`, mirrors `agent/internal/updater`'s `update-state.json` convention) — never read or written by anything except the agent service itself, so no cross-privilege ACL grant is needed anywhere in this design. **Verified live on real hardware** (device "Nebuchadnezzar," agent v0.3.0) — `agent.log` shows the pipe connecting successfully immediately after startup (`service: tray: pipe connected for session N`), zero new `Access is denied` write failures since the upgrade (versus a 16-day streak of them before), and the old `pending-reboot.json` file confirmed absent with no replacement stray file. **Not yet exercised live**: the actual reboot-prompt confirm/snooze round trip (no patch was pending on the verification device at the time) and multi-session/RDS behavior.

  **Blank-icon recovery** — bounded, in-process, no process restart: `recoverBlankIcon()` calls the fork's `systray.Readd()` (a fresh `NIM_DELETE`+`NIM_ADD` from the same still-running window) at 10s/30s/60s/120s after launch, then stops. This replaced an earlier design (a `--restart-after=10m` flag making the whole tray process exit and rely on the agent's own 60s liveness reconciliation to relaunch it) that turned out to be the actual ongoing cause of real-hardware blank-icon reports: every exit-and-relaunch cycle genuinely deregistered the icon and left it absent for up to a minute before anything re-added it — exactly the vanish-and-reappear pattern Windows' taskbar is documented to render as a blank placeholder for a recently-pinned icon. The mechanism built to fix the blank icon was, every 10 minutes for the life of every session, manufacturing the condition that causes it. The current design instead bounds recovery to the narrow real race it exists for (`WTS_SESSION_LOGON` racing Explorer's own taskbar-window creation at startup, a one-time phenomenon) rather than re-triggering the underlying Windows quirk indefinitely.

  **Init-failure resilience** — a second, distinct real bug, found by pulling `beacon-tray.log` off a real production device: `registerSystray()`'s Windows-side setup (window class registration, window creation, etc.) can fail, and upstream `fyne.io/systray` just returns silently on that failure while `Run()` still calls `nativeLoop()` unconditionally right after — leaving the process running its message loop forever with no window and no icon, looking perfectly healthy to the PID-liveness supervisor. The fork's `exitOnRegisterFailure` now calls `os.Exit(1)` on that failure instead, so the existing dead-PID relaunch logic gets a real chance to retry; each of `initInstance()`'s ~8 possible failure points also now names the specific failing Win32 call instead of one generic "Unspecified error" message. *Why* the underlying Win32 call fails at all is still unknown — the next real recurrence will finally say.

  Verified end-to-end on real hardware for the multi-session and duplicate-icon-on-update cases (the underlying `EnsureTrayRunning()`/`usersession` mechanics, unchanged by either fix above). **The two fixes above are not yet confirmed on real hardware** — full chronological history (every round of the blank-icon saga, including the disproven one-shot and periodic-restart designs) is in PROJECT_LOG.
...
- `agent/internal/audit/hardware.go` collects: BIOS serial number, last-logged-in-user, architecture, system manufacturer/model/motherboard, display adapters, installed (physical DIMM) vs. usable RAM, domain/Windows display version/installation type (Windows-only), and virtualization/hypervisor-guest detection. `agent/internal/audit/hyperv.go`'s `collectHyperVHost()` separately collects the Hyper-V-host-role flag (nil = never evaluated, never a false negative from a transient failure) feeding `devices.is_hyper_v_host` (see Patch Management's Hyper-V exclusion). All ride the existing `hardware` audit JSON blob — no migrations needed. (`ConsoleUserCanElevate` no longer exists — it was Elevate's old Administrator-token-era pre-flight signal, dropped in PR #140 alongside `launchElevatedShell`; see Web Remote's "On-demand elevation" section.)
...
Published version assets are immutable: a same-version retry verifies identical hosted bytes and skips an identical current catalog entry, but never clobbers or duplicates. Every new release uploads a detached `.sig` asset beside each binary. `agent/internal/releasekey.PublicKeyHex` owns the Beacon-project default, replaced with `-X` for a host-controlled release. Both `tools/sign`/`tools/verify` consume that same value. The manual `Publish upstream agent release` GitHub Actions workflow uses the project-owned `BEACON_PROJECT_AGENT_SIGNING_KEY` repository secret and `--upstream` to produce generic artifacts without contacting a Beacon Worker. `scripts/bootstrap-upstream-agent-channel.mjs <version>` verifies those public detached signatures against the built-in project key before registering them with a fresh host. The official Beacon source repository is reserved for that upstream channel; a host-controlled deployment must set `BEACON_RELEASE_REPOSITORY` to its own public artifact repository. Signing-key continuity is an operator contract — existing agents only accept releases signed by the private half of their embedded public key; there's no automatic key rotation.
...
Latest migration: `0085` (RustDesk on-demand install pipeline, see that section below). Check `migrations/` directly for the full ordered list; PROJECT_LOG.md has the session-by-session story for anything not covered here.
...
**Entra app registration**: redirect URI `<worker-url>/v1/auth/microsoft/callback`; two Graph permissions, both needing admin consent — `GroupMember.Read.All` (Delegated, login-time) and `Group.Read.All` (Application, used by the group-search feature below via client-credentials). `scripts/setup-entra-sso.mjs` automates this registration end to end (device-code sign-in via Azure CLI's well-known public client ID, then Graph calls for the app/permissions/service-principal/secret) short of the final admin-consent click, which it deliberately leaves as a printed one-click link rather than a scope this script grants itself — see that script's own header comment. Sign-in through app creation is cross-checked against Graph API docs but not yet exercised against a real tenant.
...
A server-class Windows device (RDS/AVD, multiple concurrent logged-in sessions) can target a specific session via `usersession.ActiveSessionDetails` + a `list_remote_sessions` command; client-class devices are unaffected (still shadow the single console session). `ActiveSessionDetails` matches `WTSActive`, `WTSConnected`, **and** `WTSDisconnected` (labeled "(Disconnected)" in the picker) — a real production bug (first real-hardware exercise of this picker) found it originally reused `ActiveSessions()`'s strict `WTSActive`-only filter, so a technician's own genuinely-logged-in RDS session reported as `WTSConnected`/`WTSDisconnected` at the moment of the check produced a false "no active sessions" result. The "Choose Session" modal (`DeviceDetailPage.vue`) also has a "Connect to Console" fallback button when the picker legitimately returns zero sessions — the modal previously dead-ended there with no way to proceed, meaning the SYSTEM fallback below could never be reached from this path at all. **Verified live on a real Windows Server 2022 VM** (PR #214) using a technician's own real, non-console RDP session as the repro: `list_remote_sessions` correctly reported `is_disconnected:false` while connected, `is_disconnected:true` after the RDP client was killed without logging off (the exact original bug — pre-fix this session would have vanished from the list entirely), and `[]` after a full logoff — confirming the "Connect to Console" fallback's trigger condition occurs exactly as designed.

### On-demand elevation ("Elevate")
Rebuilt (PR #140) around **SYSTEM token relocation**, not an Administrator-privileged token: clicking Elevate relaunches `beacon-screenshare.exe` under the agent service's own SYSTEM token via `usersession.RunAsSessionAsSystem`, with `FollowInputDesktop` capture/injection — this is what actually lets a technician see *and click* a live UAC secure-desktop prompt (an Administrator token, split or credentialed, can never open the secure desktop at all; only SYSTEM can). Opens as a **new, independent session** (a fresh relay DO instance, zero RFB protocol changes) — `WebRemotePage.vue` connects the new elevated instance in the background against an inactive screen target and only swaps/tears down the old one once the new one is proven connected, so a failed elevation attempt never loses the working session. A real dashboard-side stale-event race (the old connection's async `disconnect` event landing after the new one had already set `status='connecting'`, clobbering it) is guarded by a `connectionSeq` generation counter. The Elevate confirmation modal (`WebRemotePage.vue`) is a plain static confirmation with no pre-check of whether elevation will actually succeed on this device — the old Administrator-token design's pre-flight visibility signals (`ActiveUserCanElevate()`/`console_user_can_elevate` audit field, a company-level `hasElevationCredentials` check) were dropped in the same #140 rewrite, since SYSTEM relocation doesn't depend on the console user's own privilege level or any stored credential at all. There is also no separate elevated shell or admin-tools menu anymore: `launchElevatedShell`, which used to open a second PowerShell window with a TUI `menu` of admin destinations (Task Manager, Services, Event Viewer, etc.), was dropped in the same PR — elevating just gives the existing session SYSTEM-level access to the same desktop the technician was already looking at, and admin tools are reached through the OS's own UI (Start menu, right-click, etc.) rather than a Beacon-provided shortcut list.

**What Elevate still cannot do**: if the *end user* independently triggers their own UAC prompt outside anything Beacon initiated, that still can't reliably be seen/clicked — the secure-desktop-following capture is built around actions the technician takes through the elevated session, not ones the end user takes on their own. Steered against via UI copy, not solvable at the OS level with the current design.
...
### End-user consent (issue #86, migration `0083`)

The one remote-control requirement left unmet on issue #86's v1.0.0 milestone before this: a Web Remote session connected and started streaming with zero notification to whoever was sitting at the target machine. `companies.remote_access_consent_required` (bool, default `false` — nothing changes for an existing company until an admin opts in) sets the fleet-wide default; `devices.remote_access_consent_override` (nullable — `null` inherits the company default, `true`/`false` is an explicit per-device override) mirrors `override_class`'s exact same override-over-a-default shape. Effective value = `device.remoteAccessConsentOverride ?? company.remoteAccessConsentRequired`, resolved server-side in `POST /v1/sessions` and threaded into the queued `open_session` command as `require_consent` (same optional-field convention as `elevated`/`target_session_id`). Scoped to `screen_share` sessions only — Shell/tcp_tunnel are a different risk/visibility profile already covered by RBAC + Activity Log, deliberately not bundled in.

**Elevate re-prompts for free** — `WebRemotePage.vue`'s `elevate()` already opens a brand-new session via a second `POST /v1/sessions` call rather than mutating the existing one, so the same `require_consent` resolution naturally re-fires for an elevated open (a real, separate jump to SYSTEM/secure-desktop access) with zero special-cased code.

**Agent**: `beacon-screenshare.exe` gained `--require-consent`. When set, right after flag-parsing and *before* the relay dial, `promptConsent()` shows a blocking `windows.MessageBox` (`MB_YESNO|MB_ICONQUESTION|MB_TOPMOST`, generic wording — no plumbing exists to carry the requesting technician's identity down through the payload) on its own goroutine, bounded by a 30s `select`/`time.After` (same shape as `rfbserver.captureWithTimeout`'s goroutine+timeout pattern for a Win32 call with no native cancellation). A timeout leaves that goroutine permanently blocked on the still-open dialog — harmless, since the process exits right after and Windows destroys every window a terminated process owned, so there's no orphaned-dialog risk. Declined or timed out: `POST /v1/sessions/:id/consent` (report-token authenticated, same pattern as `.../displays`) reports the outcome, then the process exits *without ever dialing the relay* — no fast explicit-failure round trip, matching Elevate's own `ErrElevationNotAvailable` precedent of letting the browser's own state reflect it. Accepted: reports `'accepted'`, then proceeds exactly as before this feature existed.

**Dashboard**: `WebRemotePage.vue`'s new `pollConsent()` polls `GET /v1/sessions/:id/consent` every 2s while a connection attempt is pending — always safe to run, since `status` is simply `null` (a no-op) unless consent was actually required for that dispatch. A `'declined'`/`'timed_out'` result surfaces immediately (`'The user declined remote access.'` / `'No response — the remote-access prompt timed out.'`) instead of waiting out the generic 70s connect timeout; the same helper covers `attemptElevatedConnect`'s background attempt too, rejecting its promise so the decline surfaces in the Elevate modal's existing error-banner rather than a generic "failed to connect." `CompaniesPage.vue`'s create modal and `CompanyDetailPage.vue`'s edit modal both gained a "Require end-user consent for Web Remote" checkbox (mirrors the existing `patchManagementExcluded` checkbox exactly, duplicated per this codebase's per-page convention). `DeviceDetailPage.vue`'s System section gained a tri-state "Web Remote Consent" `<select>` (Inherit / Require / Don't Require) next to Warranty.

**Explicitly out of scope**: no "remember this technician/device" skip (every session-open re-prompts when required, matching the reboot prompt's own 1-hour-not-permanent snooze precedent); the prompt doesn't name the requesting technician; the "nobody logged in yet" SYSTEM fallback (PR #167) proceeds unprompted regardless of the setting, since there's no one present to ask and nothing to protect.

**Verified live against a real local `wrangler dev`/`vite dev` pair, not just type-checked**: all three effective-value resolution cases (company default off, company on + device inherit, company on + device override off) confirmed via direct D1 seeding + real `POST /v1/sessions` calls, checking the exact dispatched command payload each time. `POST`/`GET /v1/sessions/:id/consent` round-tripped via curl with a real report-token, including a 401 on a wrong token and a 400 on an invalid status value. The full dashboard reaction was confirmed via standalone Playwright against a real session: a page freshly opened on `/remote/:id` showed the normal "Connecting…" state, and within ~3.5s of a real `POST .../consent {status:'declined'}` call (simulating what the agent would send), the page correctly switched to "The user declined remote access." with no other change. The company/device toggle and override `<select>` were both confirmed to render and persist real values through the real API. **Not verified on real hardware** — the actual on-screen `MessageBox` and relay-gating behavior in `beacon-screenshare.exe` is cross-compile-checked only, same standing limitation as the rest of this feature area.
...
### Verification status
Core flow (SYSTEM relocation, `FollowInputDesktop`, Elevate reconnect-and-swap) and multi-monitor in-place switching are confirmed on real hardware with a local administrator console user. **Not yet confirmed on real hardware**: Elevate as a standard (non-administrator) console user (SYSTEM elevation doesn't depend on the console user's privilege level, so this is expected to work identically, but has never actually been exercised); file transfer (source/cross-compile-verified only); PR #167's nobody-logged-in SYSTEM fallback itself (the *trigger condition* for it — a Server-class device's session list coming back empty — is now confirmed real via PR #214's real-hardware pass, but the fallback's actual `beacon-screenshare.exe` SYSTEM-relocation behavior once triggered from that path hasn't been separately exercised). Full chronological bug-fix history (UAC-related session/injection/capture fatal-error paths found and fixed one at a time across several real-hardware passes; the Elevate credential-based Administrator-token approach that was built, tested, and then fully superseded by SYSTEM relocation; the RDP/RDG detour that was researched and ruled out; the Server-class session-picker `WTSActive`-only false-negative bug, found and fixed and verified live on real Windows Server hardware in PR #214) is in PROJECT_LOG.md.

## RustDesk

RustDesk was actually the *original* plan for Beacon's remote-control feature, before Web Remote/Remote Shell were built from scratch — see PROJECT_LOG's "Interactive Remote Shell" session, which found a hardcoded-disabled dashboard stub for an unbuilt RustDesk integration and deferred it as "a hoster's own choice of a fuller Full Remote Control tool ... a separate, later, much simpler integration (install + a deep link)." `devices.rustdeskId` (migration `0000`) sat unused for a long time before this — an additional, opt-in remote tool alongside Web Remote/Remote Shell, not a replacement for either.

### Settings (migration `0084`)
- `rustdesk_settings` — singleton (`id=1`, mirrors `branding_identity`'s shape), admin-only `GET`/`PATCH /v1/admin/rustdesk-settings`, dashboard page `RustDeskSettingsPage.vue` (`/settings/rustdesk`). Three nullable fields (`idServer`, `relayServer`, `key`) for a self-hosted RustDesk `hbbs`/`hbbr` relay/rendezvous server — all empty by default, meaning devices use RustDesk's own free public servers. Confirmed with the user: public servers are the intended target, not a self-hosted relay — keeps Beacon on its existing all-Cloudflare-serverless stack with zero new infrastructure dependency. **Not yet honored by the install pipeline below** — every install currently uses RustDesk's public servers regardless of what's set here (self-hosted import is still Phase 2b, see below).
- `companies.rustdeskEnabled` — plain boolean, default `false`, same shape as `patchManagementExcluded`/`remoteAccessConsentRequired`, checkbox duplicated in `CompaniesPage.vue`'s create modal and `CompanyDetailPage.vue`'s edit modal. No per-device override (unlike Web Remote consent) — not built ahead of a demonstrated need.

### On-demand install pipeline (Phase 2a, migration `0085`)
Confirmed trigger model: install is manual/on-demand at the point of connecting, not a separate standalone step and not automatic the moment a company enables it — matching Splashtop's and Datto RMM's own remote-tool-launch UX. A single "Connect via RustDesk" toolbar button on Device Detail (gated on `company.rustdeskEnabled`) does both jobs from one click: if `device.rustdeskId` is null, it dispatches `install_rustdesk` and polls (3s interval) until the id appears or the command reports `failed`; once set, the button instead toggles a small panel showing the id plus a "Reveal Password" action (fetched fresh each time, never preloaded with device data).

**Installer distribution** — a pinned version, not a live fetch from RustDesk's CDN at install time, matching Application Components' own trust model. `rustdesk_installer` (singleton, `object_key`/`version`/`sha256`/`sizeBytes`/`uploadedAt`) is admin-uploaded via `POST /v1/admin/rustdesk-settings/admin/installer` (raw binary + `X-File-Version`/`X-File-SHA256`/`X-File-Size` headers — SHA-256 computed **client-side** via Web Crypto, same convention `components.ts`'s application-file upload already established, since Workers can't cheaply hash a streamed R2 put) into the existing `COMPONENT_FILES` bucket, put-then-commit-then-delete-old (Branding logo's exact safety ordering). Delivered to the agent via its own dedicated grant table/endpoint (`rustdesk_installer_downloads` / `POST /v1/rustdesk-installer/download`) rather than reusing `component_file_downloads` directly — that table is FK'd to `component_files`, which the installer isn't a row of.

**The unattended password never touches `commands.payload` or `commands.result`** — both are visible verbatim in Device Detail's Command History, so anything placed there is a permanent plaintext leak, not a transient exposure. `POST /:id/commands` (`install_rustdesk` branch, self-contained with its own `db.batch()`/return rather than falling through devices.ts's shared single-insert tail, same as `jobs.ts`'s `install_msi` dispatch) generates the password, encrypts it immediately into `devices.rustdeskPassword{Ciphertext,Nonce}` (AES-GCM, same shape as `company_variables`/`email_settings`), and hands the agent only a short-lived single-use grant token (`rustdesk_password_grants`, activated on the same `queued→sent` check-in transition as the installer grant) — the agent redeems the plaintext exactly once via `POST /v1/rustdesk-password/redeem`, and it's never written to the command result on any path, including error messages.

**Agent side** (`agent/internal/executor/install_rustdesk_windows.go`, Windows-only — `install_rustdesk_other.go` stub elsewhere): download+verify the installer (reuses `client.DownloadComponentFile`'s exact shape via a new `DownloadRustdeskInstaller` method, same SHA-256+size check as `install_msi`), redeem the password, `--silent-install`, then against the *installed* `rustdesk.exe` — `--install-service`, `--password <redeemed>`, `--get-id`. The captured id is JSON-encoded straight into `CommandResult.Stdout` (`{"rustdesk_id":"..."}`), same precedent as `network_scan`'s own result reporting; `checkin.ts` parses it into `devices.rustdeskId`.

**Verified live against local `wrangler dev`/`vite dev`, not just type-checked**: installer upload + settings GET round-tripped via curl; `install_rustdesk` dispatch confirmed to atomically insert the command + both grant rows and encrypt the password immediately; both agent-facing endpoints confirmed to 404 before grant activation and correctly gate on device-credential match (403 on mismatch) once activated; a simulated check-in with a fake `{"rustdesk_id":...}` result confirmed to write `devices.rustdeskId`; the dashboard reveal-password endpoint confirmed to return the same plaintext the agent-facing redeem endpoint would. Standalone Playwright confirmed the full Device Detail flow: button → panel showing the real id → Reveal Password → real decrypted password shown. **The two real-hardware assumptions are now confirmed** (2026-09-03, via a read-only diagnostic `run_script` dispatched against two real pre-existing RustDesk installs, Nebuchadnezzar and CDNX-LT-001, both Windows, both RustDesk 1.4.9.29722256): the installed binary really does land at `C:\Program Files\RustDesk\rustdesk.exe` (confirmed both by direct `Test-Path` and independently by the uninstall registry's own `InstallLocation`, on both machines), `--install-service` really does register a service literally named `RustDesk` (`DisplayName: RustDesk Service`, confirmed `Running` on both), and `--get-id`'s output is exactly what was assumed — a bare numeric ID on its own line (`32394917` / `1835536771` respectively, two different real values on two different machines, not a copy-paste artifact) — `strings.TrimSpace` in `install_rustdesk_windows.go` already handles the trailing newline correctly with no code change needed. **What this does *not* verify**: neither install was provisioned by Beacon's own pipeline (both pre-existed, installed manually) — the actual `install_rustdesk` command end-to-end (installer upload → dispatch → agent-driven `--silent-install`/`--install-service`/`--password` sequence → id capture → dashboard reveal) still hasn't been exercised against a real device, only individually confirmed piece by piece.

### Explicitly deferred (Phase 2b)
- **Consent parity with Web Remote, design confirmed but not implemented**: Datto RMM's own "Privacy Mode" (a same-name-but-different-thing from Beacon's own Privacy Mode, which only limits audit inventory collection) is actually the same concept as Web Remote's end-user consent (issue #86, migration `0083`) — applying uniformly across every remote tool in Datto's Agent Browser, not just one. RustDesk connections don't route through Beacon's relay at all (a technician dials the RustDesk ID directly with their own separate client), so Beacon can't inject a prompt into that handshake the way `beacon-screenshare.exe` does — but RustDesk has an equivalent primitive: a **"Click" approve mode** (`kOptionApproveMode`, verified via RustDesk's own source at `flutter/lib/consts.dart` — values `password`/`click`/`both`) plus a documented-but-not-yet-located override that falls back to the permanent password when nobody's logged in, mirroring Web Remote's own SYSTEM-fallback logic exactly. The install pipeline should set `approve-mode` based on the device's existing `remote_access_consent_override ?? company.remoteAccessConsentRequired` — reusing that toggle rather than adding a second, RustDesk-specific one. The exact headless mechanism to set this option (likely `rustdesk.exe --option approve-mode <value>`, confirmed as a real flag, but not yet exercised) needs verification against a real client.
- Self-hosted server config import (`rustdesk_settings` → `--option`/`--import-config`) — same "verify against a real client first" bar.
- Hiding the tray icon (decided: yes, hide it — RustDesk shows one to any logged-in session by default, same "always there" model as Beacon's own tray; exact config key not yet located in the source files checked).
- Non-Windows targets.
...
**Narrowed to genuine Server-OS Hyper-V hosts (migration `0082`)** — `vmms` presence alone doesn't distinguish the Server Hyper-V role from the optional Hyper-V *Platform* feature on an ordinary Windows Client desktop (WSL2, Docker Desktop, local dev VMs), so the exclusion was silently catching Client machines too, with real-world fleets showing plenty of them. `devices.windows_installation_type` (nullable TEXT — `"Client"`/`"Server"`/`"Server Core"`/etc., rides the existing `hardware.windows_installation_type` audit field, same tri-state "only set when actually sent" discipline as `is_hyper_v_host`) narrows the check to `isHyperVHost && windowsInstallationType !== 'Client'` — a confirmed Client install is no longer excluded at all; an unaudited-since-upgrade device (`windowsInstallationType` still `null`) stays conservatively excluded until a fresh audit resolves it, matching the pre-existing safe-default behavior. `DeviceDetailPage.vue`'s Hyper-V Host row reflects the same condition (`hyperVPatchExcluded`), rather than just echoing `isHyperVHost`, so a Client-OS Hyper-V machine correctly shows as included in normal targeting instead of the stale "excluded" note. Verified live against local `wrangler dev`: three seeded Hyper-V-flagged devices (Client / Server / unaudited installation type) dispatched to `manage`/`revert`/`revert` respectively on the same unrestricted Server-class policy, and an explicit Device target still overrides the exclusion for a real Server-OS host as before.
...
POST /v1/admin/devices/:id/commands          Queue a command (run_script, reboot, run_audit, restart_agent,
                                              ...
                                              uninstall_software, list_remote_sessions, install_rustdesk)
GET  /v1/admin/devices/:id/commands          Last 50 direct commands for this device (jobId IS NULL)
POST /v1/admin/devices/:id/fast-poll         Manually arm Fast Poll ahead of an upcoming session/command
GET  /v1/admin/devices/:id/rustdesk-password Reveal the device's decrypted RustDesk unattended password
...
POST /v1/rustdesk-installer/download         Agent-facing: same grant shape as above, for the pinned RustDesk installer
POST /v1/rustdesk-password/redeem            Agent-facing: one-time redemption of a device's generated unattended password
...
GET/PATCH  /v1/admin/rustdesk-settings       Global relay/rendezvous server config + pinned installer metadata (admin only)
POST /v1/admin/rustdesk-settings/admin/installer  Upload the pinned RustDesk installer (admin only)
...
POST /v1/sessions/:id/consent                Agent-facing: report Accept/Decline/timeout (report-token auth)
GET  /v1/sessions/:id/consent                Technician-facing: poll the consent outcome
