...
- **Execution-context switching** ("run as a logged in user") — shown in the UI (disabled) rather than omitted, since the segmented control itself is meaningful documentation of the gap, but not implemented: needs real Windows user-impersonation in the agent, which `agent/internal/executor` doesn't use. The underlying primitive now exists (`agent/internal/usersession`, see below) but isn't wired into the job executor yet — a real, separate follow-up, not bundled with the primitive itself.
