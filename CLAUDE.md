...
**Not yet resolved**: in one real test, killing the tray process (`Stop-Process -Name beacon-tray -Force`) did not result in it being relaunched by the next check-in tick as expected. The running service process's own log also showed *no* activity at all during this test — not even its own startup banner, which logs synchronously near the top of `main()` well before any tray logic runs — while `Get-Service`/`Get-Process` confirmed the service process was genuinely alive in session 0 the whole time. Reviewed the relevant code path (`main.go`'s check-in loop, `EnsureTrayRunning`, `setupLogging`) and found no logic bug that would explain total log silence from a definitely-running process — this needs further live investigation to actually root-cause, not further guessing. Flagging honestly as open rather than claiming it's resolved.
...
3. **Not yet resolved**: after a genuine physical-console login, `beacon-tray.exe` launched into the correct session (confirmed via `Get-Process`: correct `SessionId`, live PID) but its icon never rendered in the notification area — not hidden in the overflow flyout either, just a blank reserved slot. Process-level launch targeting is correct; something about how the icon registers with Explorer's shell notification area when launched via the impersonated `CreateProcessAsUser` path (vs. a normal interactive login) appears to be the gap. Not root-caused yet — tracked as a follow-up, not blocking PR #40 itself since Job execution (the actual feature under test) doesn't depend on the tray at all.
...
One self-contained script, run in a single invocation: builds all 5 platform/arch binaries (`-trimpath`, so the same source always produces the same bytes regardless of which machine/directory builds it — signatures stay reproducible), creates the GitHub release and uploads the binaries (idempotent — `gh release create` if the tag doesn't exist yet, `gh release upload --clobber` if it does), signs each binary with `BEACON_SIGNING_KEY`, registers each with the worker via `POST /v1/admin/agent/versions`, then re-downloads each asset from the live GitHub release and re-verifies its signature before declaring success. Needs `BEACON_WORKER_URL`, `BEACON_ADMIN_SECRET`, `BEACON_SIGNING_KEY` env vars and an authenticated `gh` CLI. **Must be run from the repo root**, not `agent/` — it resolves `scripts/publish-agent.mjs` and `agent/` relative to its own script location, but the working directory still matters for the relative build paths it shells out to.
...
No separate "create the release first" step and no `BEACON_DOWNLOAD_BASE_URL` env var — both were true of an older split flow (build+release without the secret on one machine, sign+register afterward on another) that an overhaul commit (`83040a6`, "Overhaul publish-agent.mjs: trimpath, auto GitHub release, verification") superseded once `-trimpath` made rebuilds byte-reproducible across machines, removing the reason for the split. The script computes its own correct download URL from the repo's own org/name — nothing to configure.

The script's own built-in verify step (re-download + re-check signature) is a real safety net, but **independently re-verify anyway before considering a release shippable** — it's cheap insurance against `tools/sign` not validating that the entered key's derived public half matches `pinnedPublicKey` (nothing in the codebase checks this, so a wrong/corrupted key signs successfully and every step — build/sign/register/download/verify — reports success anyway; the only real-world symptom is devices silently never updating, invisible on Windows since services have no console). Also spot-check end-to-end via the **unauthenticated** `GET /v1/agent/version?os=&arch=&current=` and `GET /v1/agent/download?os=&arch=` endpoints (`worker/src/routes/agent-update.ts`) — a `curl -sIL` through `/v1/agent/download` should end in `HTTP/2 200`.

**`BEACON_SIGNING_KEY` has no `.env`/secrets-manager home in the current workflow — it's manually re-entered each release from the password manager**, a real, repeated source of paste errors. **Open question, not yet resolved**: whether this manual-paste-per-release step should be replaced with something that doesn't require re-entering the key each time (a password-manager CLI integration, a locally-encrypted key file, etc.) — worth revisiting given how often this step alone has derailed a release.
...
Auto-approval rules (a `min_severity` threshold — deliberately simple, no category/KB-pattern filter builder, matching how CPU/memory monitors use a plain threshold rather than a rule engine) plus scheduled recurring install windows, modeled on Maintenance Policy's targeting/recurrence shape (`maintenance_policies`, `worker/src/lib/maintenance.ts`, migration `0050`) but **not** its runtime model: Maintenance Policy is a passive suppression gate checked on-demand at alert-evaluation time; Patch Policy *actively dispatches* `install_patches` commands at each scheduled window, closer to Jobs' `dispatchDueScheduledJobs` cron-dispatch shape (`worker/src/routes/admin/jobs.ts`) than to Maintenance Policy's `isMaintenanceWindowActive`.

- **Tables**: `patch_policies` — same recurrence columns as `maintenance_policies` (duplicated verbatim, not shared, matching this codebase's established per-policy-type mirroring convention), plus `min_severity` (nullable TEXT `'Critical'|'Important'|'Moderate'|'Low'` — `NULL` means no auto-approval rule, only manually-approved patches ever install), `auto_reboot` (bool, default `false`), and `last_dispatched_at` (nullable INTEGER — see dispatch-tracking below). `patch_policy_companies`/`patch_policy_devices`/`patch_policy_groups` — the exact same 3-table composite-PK OR-list targeting shape as `maintenance_policy_{companies,devices,groups}` (empty = matches every device).
...
- **Auto-approval is fleet-wide, same as manual approval** — `patch_approvals` has no per-device concept, so a policy's `min_severity` threshold, if met by any patch present on any device anywhere in the fleet, triggers global approval of that patch (`autoApprovePatches`, reusing `PATCH /v1/admin/patches/:updateId`'s exact check-then-insert upsert shape, not `onConflictDoNothing` — no upsert precedent for that exists elsewhere in this codebase). The policy's own targeting only controls which devices actually *receive* the resulting install, not which patches get approved. `SEVERITY_RANK: {Critical: 4, Important: 3, Moderate: 2, Low: 1}` is the one place order is imposed on an otherwise free-text `severity` column.
...
- **Dashboard**: `PatchPoliciesPage.vue`/`PatchPolicyFormPage.vue` (`/global/patch-policies`, `/global/patch-policies/new`, `/global/patch-policies/:id`) mirror `MaintenancePoliciesPage.vue`/`MaintenancePolicyFormPage.vue` exactly (same Targets flyout, same seg-bar schedule picker), plus two sections Maintenance Policy doesn't have: **Auto-Approval** (severity `<select>`: Off/Low/Moderate/Important/Critical) and **Auto Reboot** (Disabled/Enabled seg-bar, defaulting Disabled). Sidebar: "Patch Policies" link in the **Global** section, after Patches.
- **Explicitly out of scope**: anything beyond a single severity-threshold rule (no category/KB-pattern filter builder), per-target enable/disable overrides, patch rollback/uninstall on failure, per-update retry logic, any Shared Dashboards widget.
...
- **Real-hardware verification, two real bugs found, one fully fixed**: dispatched against a real Windows box, the service and its registry entry both cleanly disappeared, but `C:\Program Files\Beacon` was left behind with the tray icon still visibly running. Root cause: `beacon-tray.exe` runs as a *separate* process launched into the logged-in user's own session (`usersession.RunAsSession`), not a child of the service being stopped — its own open file handle on `beacon-tray.exe` blocks `rd /s /q`. Fixed with `taskkill /IM beacon-tray.exe /F` in the helper script (SYSTEM's `SeDebugPrivilege` reaches a process in a different session; `/IM` catches every instance across every session, covering RDS/multi-session too). **Retested after that fix and the directory was still left behind** — a second, suspected-but-not-yet-confirmed cause: the helper's `timeout /t 3` step needs a real console input handle even with `/nobreak`, and this helper runs with `CREATE_NO_WINDOW` from a SYSTEM service with no console at all, so `timeout` may fail immediately, collapsing the intended delay to zero and racing `rd /s /q` against this process's own in-progress exit. Switched to `ping -n 4 127.0.0.1 >nul` (the standard no-console-needed batch-scripting delay idiom) — **not yet re-verified on hardware**, flagging honestly rather than claiming this is confirmed fixed. The self-referential stop/restart race the design was originally built to avoid was never actually hit live — both real bugs found were simpler oversights (an unrelated process holding a lock; a delay primitive that doesn't work in a headless context), the kind only real hardware turns up.
