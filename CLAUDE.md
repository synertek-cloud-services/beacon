...
- **Custom Fields ("UDF" equivalent) shipped** — dynamic admin-defined named fields with per-device values, deliberately not Datto's 300 fixed numbered slots (scoped against the Datto UDF spec in a same-day session that ran out of context before implementing anything beyond the migration). Migration `0029_custom_fields.sql`: `custom_fields` (id, name, sort_order) + `device_custom_field_values` (a real join table, composite PK, `ON DELETE CASCADE` both directions — not a JSON blob, so a future filter/targeting pass won't need a schema change). Worker: `worker/src/routes/admin/custom-fields.ts` (new, admin-only field-definition CRUD) + `GET/PATCH /v1/admin/devices/:id/custom-fields[/:fieldId]` in `devices.ts` (readonly to view, technician to set — upsert-by-check-then-insert-or-update). Dashboard: `/settings/custom-fields` (`CustomFieldsSettingsPage.vue`, admin-only, modeled on `SsoSettingsPage.vue`'s group-mappings list — inline-editable name, ↑/↓ reorder persisting `sort_order` immediately) and a new **Custom Fields** section on `DeviceDetailPage.vue` between Network and Security (matching Datto's own relative UDF placement), one inline-editable text input per field. Manual entry only this pass — no Job/Policy targeting by field value, no agent-write capability (both explicitly deferred, matching Datto's fuller spec). A real bug was caught only by browser-testing (not `vue-tsc`): the new refs were declared textually after `onIdChange`, which the page's `watch(..., {immediate:true})` invokes synchronously during `<script setup>` execution, throwing a TDZ `Cannot access before initialization` error on every device-page load — fixed by moving the declarations up next to the other section-state refs `onIdChange` already reads. **`0029` was pushed and migrated to production before this follow-up work started.**
- **Custom Fields made usable as script variables** — closes the gap with Datto's real UDF system, researched directly against Datto's own docs (rmm.datto.com) this session: Input Variables (Beacon's `component_variables`) and UDF variables are genuinely separate mechanisms there, and the user confirmed Beacon should match the UDF shape (reference-by-name, no per-component declaration) rather than bind-at-creation-time. Migration `0030_custom_fields_key.sql` adds `custom_fields.key` (identifier form of `name`, partial-unique on non-empty). A script now references `${CF_<KEY>}` (bash) / `$env:CF_<KEY>` (PowerShell) / `%CF_<KEY>%` (Batch), resolved fresh **per-device** at job dispatch time inside `insertJobCommands` (`jobs.ts`) — proved via a real test dispatching one job/component to a Windows and a Linux device simultaneously and confirming each device's queued command payload carried its own distinct value. Renaming a field's `key` is blocked (409, not silently allowed) if any `components.script` still contains the old `CF_<OLDKEY>` string — a plain JS substring scan, deliberately not SQL `LIKE` (key values contain `_`, which `LIKE` treats as a wildcard). No agent-side change needed — `run.go` already treats `variables` as an opaque flat env map. Full design/verification detail in "Custom Fields" below.
- **Sidebar collapsed state redesigned to a 44px icon-only rail with flyout submenus** — replaces the previous fully-hidden (0px) collapse. When collapsed, `App.vue` adds `.collapsed` to the `<nav>`, CSS hides all labels/chevrons/sec-body, and section icons are centered. Clicking a section icon calls `handleSectionClick()`, which sets `openFlyout` (a `string | null` ref) and records `flyoutTop` from `getBoundingClientRect().top`. A `position: fixed; left: 44px` `.nav-flyout` panel renders the section's sub-items (full markup duplicated inside the flyout, consistent with the codebase's per-component duplication convention). A `.flyout-backdrop` (`position: fixed; inset: 0; z-index: 598`) sits behind it and closes the flyout on click. Toggle button repositioned to `left: 44px` (was `left: 11px`) when collapsed so it still straddles the sidebar's right edge. Route-change watch also closes the flyout. Expanding the sidebar clears `openFlyout`. All accordion behavior in expanded mode unchanged.
- **Single Alert Detail page shipped** (`/global/alerts/:id`, `dashboard/src/pages/AlertDetailPage.vue`) — Datto-style Single Alert View. Three sections: **Overview** (2-column `.ad-grid`: Message/Created/Status/Alert ID left; Device/Company/Policy/Monitor Type right), **Timeline** (vertical event spine derived from `alerted_at`/`acknowledged_at`/`resolved_at` timestamps, no new data model), **Device Alerts** (same-device alert list via existing `device_id` filter, current alert highlighted with `.ad-row-current`). Topbar: priority badge + Acknowledge + Resolve action buttons. Acknowledge does an optimistic local update (hides button immediately, sets `acknowledged_at` locally). Resolve calls API then navigates back. Worker endpoint `GET /v1/admin/alerts/:id` added — must be registered *before* `/:id/resolve` in Hono's route table (order matters). Alert row clicks in **GlobalAlertsPage**, **DeviceDetailPage**, and **OverviewPage** all navigate to this detail page; checkboxes still work via `@click.stop`.
- **Alert 3-state status** — DeviceDetailPage's alert mini-table updated to show Open/Acknowledged/Resolved: `!a.is_alerting ? 'status-resolved' : a.acknowledged_at ? 'status-acked' : 'status-open'`. Migration 0027 (`acknowledged_at`/`acknowledged_by` on `alert_state`) was missing from production D1 and had to be applied manually before this worked.
- **`target_os` on components** (migration `0028_component_target_os.sql`) — `components.target_os TEXT DEFAULT NULL`. `null` = all platforms; `'windows'`/`'linux'`/`'darwin'` = OS-specific. Filtering applied per-device at dispatch time inside `insertJobCommands` (`worker/src/routes/admin/jobs.ts`) — incompatible components skipped per device, device skipped entirely if no compatible components remain. ComStore built-ins tagged appropriately (Windows temp-clear → `windows`, Linux restart/reinstall → `linux`). `ComponentFormPage.vue` exposes a Platform `<select>` (disabled on `origin='store'` components via `isStore` ref set on load). CRUD routes (`components.ts`) propagate `target_os` through create, update, and clone.
- **Linux agent-restart scripts fixed** (in migration 0028) — original ComStore scripts called `systemctl restart beacon-agent` directly, which kills the agent process (the script's parent) before it can report the result, leaving the job permanently in "Running" state. Fixed by backgrounding the restart with a 5-second delay: `nohup sh -c 'sleep 5 && systemctl restart beacon-agent' >/dev/null 2>&1 &`. This detaches the restart from the current process tree, giving the agent time to check in and report success before dying. This pattern applies to any ComStore script that kills the agent itself.
- **Real scheduled job dispatch shipped, and Create Job moved to a full page.** `jobs.scheduled_at`/`expires_at`/`run_as_system` had existed in the schema since the original design but were entirely dead code — `POST /v1/admin/jobs` accepted a `type: 'scheduled'` job, inserted the row, and then dispatched nothing, ever; there was no cron path to pick it up. Fixed: `worker/src/routes/admin/jobs.ts` now exports `dispatchDueScheduledJobs`/`cancelExpiredScheduledJobs`, wired into the existing 2-minute cron in `index.ts`. Target devices for a scheduled job resolve **at dispatch time, not creation time** — matches Datto's own documented "devices targeted by a Job are calculated just before it is scheduled to run" semantics, since the matching device set can legitimately change before a future `scheduled_at` arrives. A job that expires before ever resolving any devices is cancelled rather than left dispatching late. `CreateJobModal.vue` (quick-job-only, never updated since the original design) is deleted; `dashboard/src/pages/JobFormPage.vue` (`/jobs/new`) replaces it as a full page mirroring `ComponentFormPage.vue`/`PolicyFormPage.vue`'s shell, with real Schedule (Immediately vs. a scheduled time + expiration) and Execution sections. "Run as a logged in user" is shown but **disabled** — the agent has no Windows user-impersonation capability (`WTSQueryUserToken`/`CreateProcessAsUser`-style) built anywhere, so faking that toggle would misrepresent a real capability gap. Notification (email) is left out of the page entirely — Beacon has no email infrastructure at all. Full data model and dashboard details in "Components / Job System" below. Verified end-to-end: quick-job regression (still dispatches immediately), scheduled-job dispatch (via `wrangler dev`'s `/cdn-cgi/handler/scheduled` manual cron trigger), expiration-cancels-undispatched, and the real `/jobs/new` UI flow via Playwright against a real local `wrangler dev` + D1.
- **Production `ADMIN_SECRET` rotation surfaced a real, unrelated production incident**: while chasing why a WSL2 dev-machine agent hadn't picked up v0.2.8, `systemctl status beacon-agent` + `agent.log` showed the process had been running continuously since a prior day with **no crash and no error**, but its self-update goroutine's 24h check simply never fired again after its first run. Root cause: Go's `time.Sleep(24h)` doesn't reliably count time while the WSL2 VM is suspended (laptop sleep) — the goroutine's internal clock drifted behind real wall-clock time, so two full days passed without a second version check ever firing. Not a code bug (the v0.2.5 `awaitConfirmation` fix from a prior session is unrelated and still correct) — a real environmental gotcha worth remembering for any WSL2/laptop-suspend dev or test agent: `systemctl restart beacon-agent` resets the 5-minute stagger and forces an immediate fresh check, which is the fix, not a code change. Confirmed the restart took the box straight from v0.2.6 → v0.2.8 in under a second once unstuck.
...
- **Production `ADMIN_SECRET` rotated 2026-07-16** (had been exposed in a session transcript since 2026-07-14). Generated and set via `wrangler secret put` reading from a private, restrictively-permissioned scratch file — the value never appeared in any tool output or chat text, same handling as the Ed25519 signing key. Verified end-to-end with an authenticated call against production before the scratch file was deleted.
...
- **Job Detail page shipped** (`dashboard/src/pages/JobDetailPage.vue`, `/jobs/:id`) — breadcrumb/title bar with Retire/Purge buttons; Details card (2-column grid, `.jd-details-grid`); an inline SVG flow diagram showing the Pending→Running→Successes/Warnings/Failures pipeline with dynamic colors keyed to live command counts; Devices table with per-command StdOut/StdErr inline expansion (one open at a time, toggleable). `JobsPage.vue` row click now navigates to this page; all prior inline expansion code (`expandedId`, `detail`, `detailLoading`, the `CmdResult` interface, `toggleExpanded`, `cancelJob`) removed. Worker `GET /v1/admin/jobs/:id` extended to include `c.warning` in the command SELECT (stored as SQLite integer, coerced `row.warning === 1` in the route handler). `warning` field drives both the `.jd-status-warning` badge on the detail page and the `.mini-warning` badge on the jobs list — both call a `badgeClass(cmd)` helper (class+label change together) rather than a template ternary.
- **Flyout selected-state pattern made consistent** — both the Component picker flyout (`.cf-`) and the Target picker flyout (`.tf-`) now use identical interaction: accent left border + background tint on the row, **teal checkmark on the right replacing the Add button** (`v-if/v-else`), checkmark is clickable to remove. Previously the component flyout used a separate Remove button after Add; the target flyout used a left-side checkbox. Corrected mid-session after user feedback. See STYLE.md for the exact pattern.
- **Target flyout rebuilt** (`JobFormPage.vue`) — replaced a 3-step type-picker modal flow with a Datto-style right-side panel: a `<select>` category dropdown (All Devices / Sites / Devices), search input, per-row Add/checkmark. `toggleTarget()` handles kind-switching (adding a site target clears any existing device targets, and vice versa). `isTargeted(kind, id?)` drives both per-row state and the selected-target chips shown on the form.
- **Quick Job modal updated** (`DeviceDetailPage.vue`) — added a ComStore tab alongside the existing Library and Write Script tabs, matching `ComponentsPage.vue`'s My Library / Browse Store split. Store components loaded lazily on first tab open, same pattern as library components.
