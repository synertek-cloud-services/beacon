...
- **Agent is at v0.2.6.** Six releases landed across two consecutive sessions (v0.2.2 → v0.2.6) — see "Agent release process" below before cutting another one. It has two real gotchas now, both bit production this cycle: the dead-placeholder-download-URL issue (below) and a signing-key corruption incident (below) that silently invalidated every v0.2.2 signature. **Every release must now be independently re-verified** (download the real GitHub asset, re-run the Ed25519 check against the registered `signature_hex`) before being considered shippable — a successful `/v1/admin/agent/versions` POST is not proof the signature is actually valid.
...
- **Migrations `0020`–`0022` and the corresponding worker deploys are done in production** (variables/ComStore/post-conditions, the superseded single-site attempt, and the real multi-site table respectively) — the dashboard side rode Cloudflare Pages' existing auto-deploy. **The agent itself was never rebuilt or released this session** — `agent/cmd/agent/main.go`'s `version` is still `"0.2.6"` despite two real Go changes landing (`executor/run.go`'s variable→env-var injection, `hardware.go`'s virtualization detection). Neither feature does anything on a real device until a `v0.2.7` is built, signed, and released through the normal process — see PROJECT_LOG.md's "Next logical steps."
...
**Known gotcha — the registered `download_url` is a dead placeholder.** The script's default `downloadUrl` is `${workerUrl}/dist/<name>`, which nothing actually serves (no R2/CDN hosting is configured) — the version registers fine and `update_available` correctly flips to `true`, but any agent attempting the download gets a 404 and never actually updates. **Avoid this entirely by creating the GitHub Release *before* registering anything**: `gh release create vX.Y.Z dist/beacon-agent-*` first, then sign + `POST /v1/admin/agent/versions` directly with the real `.../releases/download/vX.Y.Z/<asset>` URL. v0.2.0 through v0.2.2 didn't do this and needed a two-step register-placeholder-then-re-register-with-real-URL dance (reusing the same `signature_hex`, since the signature covers binary bytes, not the URL); v0.2.3 through v0.2.6 created the release first and skipped the dance entirely. `publish-agent.mjs` itself still defaults to the placeholder — either build the GitHub-release-first flow into it, or keep doing it by hand as above.
...
**One continuous scrollable page, not tabs.** Explicitly corrected after an initial tabs-based (`v-if`/`v-else-if`) implementation — user feedback: "it is still supposed to be one page. The links just make it quicker to navigate." Nav order (matching a real Datto RMM device page exactly, shown mid-build): **Summary → System → Alerts → Policies → Software → Services → Memory → Storage → Network → Security → Change Log**. All render simultaneously as `<section :id="'ddev-sec-' + name" class="ddev-page-section">` blocks in that same DOM order; the left-nav only scrolls to and highlights a section, it never hides content. See STYLE.md for the section-separation/scroll-spy CSS+JS pattern in full.
...
- **Network**: adapter name/MAC/IPs. **Gap, not yet built**: Datto's reference also shows an External IP field Beacon doesn't collect at all — scoped as a follow-up, cheapest implementation is worker-side (capture the check-in request's own source IP) rather than an agent-side outbound call.
