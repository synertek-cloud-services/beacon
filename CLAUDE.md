...
- `agent/internal/usersession` (Windows-only, `usersession_other.go` no-op stub elsewhere) — `RunAsActiveUser(exe, args)` launches a process in the currently logged-in console user's own session/environment from the agent's SYSTEM-context service process (`WTSGetActiveConsoleSessionId` → `WTSQueryUserToken` → `DuplicateTokenEx(TokenPrimary)` → `CreateEnvironmentBlock` → `CreateProcessAsUser`). Built as a standalone, reusable primitive after "the agent is headless" had separately blocked three different asks (Job System's declined "run as a logged in user," Patch Management's declined reboot-prompt, and implicitly anything else wanting to show a signed-in user something) — deliberately shipped with **no consumer wired up yet**, on its own, so its security-sensitive review surface (token/handle lifecycle) stays small and independently auditable. Doesn't expand what Beacon can already do — every command already runs as SYSTEM, strictly more powerful than any user's own context; this only changes *whose* context a launch happens in. `ErrNoActiveSession` is the expected, common case (nobody has ever logged into the console, or a server with no interactive session) — always a no-op for callers, never a surfaced failure. **Locking the screen does not trigger this** — `WTSGetActiveConsoleSessionId` still returns the same session and `WTSQueryUserToken` still succeeds, since locking never disconnects the session; a launched process would just sit behind the lock screen until unlock. v1 targets only the active console session, not every RDP/fast-user-switched session on a terminal server.
