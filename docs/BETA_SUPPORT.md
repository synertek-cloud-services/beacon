...
| Windows pending-reboot breadcrumb | `reboot-pending.json`, same directory as `agent.log` | Only present when a patch install left `RebootRequired=true` and the tray prompt hasn't been answered/actioned yet — a SYSTEM-only restart-survival breadcrumb, not the live handshake itself (that now flows over `agent/internal/traypipe`'s named pipe; see `agent/internal/service/reboot_windows.go`). |
