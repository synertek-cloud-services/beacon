...
| Interactive remote shell | Unvalidated | Experimental | Unvalidated | Hosted binary forwarding, both connection orders, DO hibernation, and real Linux PTY cleanup now pass in isolation. A continuous dashboard-to-enrolled-agent rerun remains the Linux promotion gate; tracked in [#98](https://github.com/synertek-cloud-services/beacon/issues/98). |
