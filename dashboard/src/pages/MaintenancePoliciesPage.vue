<template>
  <div class="gp-page">
    ...
    <div class="gp-header">
      <h1 class="gp-title">Maintenance Policies</h1>
      <div class="gp-header-actions">
        <button class="btn btn-primary btn-sm" @click="router.push('/global/maintenance-policies/new')">Create Policy</button>
      </div>
    </div>
    ...
    <div class="gp-content">
      ...
      <div v-else-if="policies.length === 0" class="gp-state">No maintenance policies configured.</div>
        ...
        <table class="policy-table">
          ...
          <tbody>
            <tr
              ...
              @click="router.push('/global/maintenance-policies/' + policy.id)"
            >
...
import { api, type MaintenancePolicy } from '../api';
...
async function load() {
  loading.value = true;
  try { policies.value = await api.maintenancePolicies.list(); }
...
function editSelected() {
  ...
  router.push('/global/maintenance-policies/' + [...selectedIds.value][0]);
}
...
async function bulkDelete() {
  ...
  try {
    await Promise.all(ids.map(id => api.maintenancePolicies.delete(id)));
...
async function togglePolicy(policy: MaintenancePolicy) {
  try {
    await api.maintenancePolicies.update(policy.id, { enabled: !policy.enabled });
...
function scheduleSummary(p: MaintenancePolicy): string {
...
.col-targets  { width: 200px; }
.col-schedule { width: 260px; }
