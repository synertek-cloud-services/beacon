<template>
  <div class="pf-page">
    ...
    <nav class="pf-crumb">
      <RouterLink to="/global/patch-policies" class="pf-crumb-link">Patch Policies</RouterLink>
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
      <span class="pf-crumb-current">{{ isNew ? 'Create Policy' : 'Edit Policy' }}</span>
    </nav>

    <div class="pf-topbar">
      <button class="pf-back" @click="router.push('/global/patch-policies')">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <h1 class="pf-title">{{ isNew ? 'Create Policy' : (form.name || 'Edit Policy') }}</h1>
      <div class="pf-topbar-right">
        <button class="btn btn-ghost btn-sm" @click="router.push('/global/patch-policies')">Cancel</button>
    ...
    <div v-else class="pf-body">
      ...
      <!-- Auto Reboot -->
      <div class="pf-group">
        <label class="pf-label">Auto Reboot</label>
        <p class="field-hint" style="margin-top:-4px">
          When enabled, a device that reports a pending reboot after this policy's install skips the
          interactive Restart Now/Postpone prompt and reboots immediately. When disabled (default), the
          device shows the normal tray notification instead.
        </p>
        <div class="seg-bar">
          <button :class="['seg-btn', { active: !form.autoReboot }]" @click="form.autoReboot = false">Disabled</button>
          <button :class="['seg-btn', 'seg-primary', { active: form.autoReboot }]" @click="form.autoReboot = true">Enabled</button>
        </div>
      </div>
...
async function toggleTarget(item: PatchTargetItem) {
...
async function save() {
  ...
  try {
    if (isNew.value) {
      ...
      await api.patchPolicies.update(policyId.value, {
        name: form.name,
        description: form.description || null,
        enabled: form.enabled,
        recurrence: buildRecurrenceBody(),
