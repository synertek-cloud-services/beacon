...
function openSettings() { if (!dashboard.value) return; settingsName.value = dashboard.value.name; settingsCompanyIds.value = [...dashboard.value.companyIds]; settingsHome.value = dashboard.value.isHome; showSettings.value = true; }
watch(showSettings, shown => { if (shown) openSettings(); });
async function saveSettings() { if (!dashboard.value) return; const updated = await api.dashboards.update(dashboard.value.id, { name: settingsName.value, companyIds: settingsCompanyIds.value, isHome: settingsHome.value }); dashboard.value = updated; showSettings.value = false; await loadList(); window.dispatchEvent(new Event('beacon:dashboards-changed')); }
