...
**6. Components: "Requires Admin to Run" gate (PR pending, migration `0069`)**
...
1. **Release + real-hardware verification of the tray blank-icon fix** — needs `publish-agent.mjs`, then a real reboot to confirm the self-heal actually works.
...
3. **Land the pending "Requires Admin to Run" PR** — code-complete and verified end-to-end against local `wrangler dev` with a real technician session, not yet committed/PR'd as of this log entry.
