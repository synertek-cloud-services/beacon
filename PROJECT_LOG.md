...
7. **Deploy migration 0030 + the worker to production** — `0029` is already live, but `0030` (the `key` column) and every worker/dashboard change in item 8 above are still local-only as of end of session. Needs `wrangler d1 migrations apply beacon --remote` + `wrangler deploy`, plus a commit/push, before script-variable referencing works anywhere but local dev.
