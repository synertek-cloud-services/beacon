...
// Readd re-registers the tray icon (a fresh delete-then-add) without
// tearing down the window or process. Beacon fork addition, not upstream --
// see systray_windows.go's reAdd for the Windows notification-area
// blank-slot problem this exists to solve without needing a full process
// restart. No-op on platforms without an equivalent failure mode.
func Readd() {
	reAdd()
}
