...
// reAdd re-registers the current tray icon with a fresh NIM_DELETE followed
// by NIM_ADD, without tearing down the window or process -- the in-process
// equivalent of what a full restart-and-relaunch was previously used to
// achieve. SetIcon/SetTooltip (NIM_MODIFY) can't repair a notification-area
// slot that registered before Explorer's own taskbar window existed (a
// modify assumes the slot is already correctly registered); only a fresh
// NIM_ADD forces Explorer to re-evaluate it. Beacon fork addition -- not
// upstream.
func reAdd() {
	wt.muNID.Lock()
